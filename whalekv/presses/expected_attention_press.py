...
from whalekv.utils import get_prerope_query_states
...
class ExpectedAttentionPress(ScorerPress):
    ...
    Algorithm:
        ...
        2. Compute the RoPE rotation matrix R over the next n_future_positions and average it.
        3. Apply R to the mean and covariance matrix of the queries.
        4. As attention A = exp(Q @ K / sqrt(d)), compute the expected attention:
    ...
    n_sink : int, default=4
        ...
        Preserves first few tokens due to the "sink attention" phenomenon where
        models assign high attention to early tokens regardless of semantic importance.
    ...
    def score(
        ...
        # Add back the sink tokens with max score to prevent pruning
