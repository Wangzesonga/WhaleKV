...
class SnapKVPress(ScorerPress):
    ...
    def compute_window_attention(module, hidden_states, keys, window_size, position_embeddings):
        """
        Compute the last window_size queries and associated attention weights
        for the first q_len - window_size keys.
        """
    ...
    def score(
        ...
        if hidden_states.shape[1] <= self.window_size:
            return torch.ones(bsz, num_key_value_heads, k_len, device=keys.device, dtype=keys.dtype)
        ...
        # Add back the observation window with max score to prevent pruning
